...
contract('exchange', (accounts) => {
  ...
  before(async () => {
    ...
    console.log('exchangeInstance Address', exchange.address);
  });
  ...
  describe.skip('유동성 공급', async () => {
    it('유동성 공급한다.', async () => {
      // exchange 컨트랙트에게 10개의 WEMEX토큰을 사용할 수 있도록 승인
      await token.approve(exchange.address, web3.utils.toWei('10', 'ether'));
      // token.transferFrom(msg.sender, address(this), _amount);

      // 유동성 공급자는 10개의 WEMEX 토큰과 2개의 이더리움을 Exchange 컨트랙트에 유동성 공급한다.
      await exchange.addLiquidity(web3.utils.toWei('10', 'ether'), {
        value: web3.utils.toWei('2', 'ether'),
      });
      // Exchange 컨트랙트의 잔고는 2이더가 있어야한다.
      let exchangeBalance = await exchange.getBalance();
      expect(web3.utils.toBN(exchangeBalance).toString()).to.equal(
        web3.utils.toWei('2', 'ether')
      );

      //  Exchange 컨트랙트에 들어간 토큰 개수는 10개여야한다.
      var balance = await token.balanceOf(exchange.address);
  ...
  describe.skip('스왑 ', async () => {
    it('사용자가 이더를 주면 그 양만큼 토큰을 준다.', async () => {
      // exchange 컨트랙트에게 100개의 WEMEX토큰을 사용할 수 있도록 승인
      await token.approve(exchange.address, web3.utils.toWei('100', 'ether'));
      // 유동성 공급자는 70개의 WEMEX 토큰과 10개의 이더리움을 Exchange 컨트랙트에 유동성 공급한다.
      await exchange.addLiquidity(web3.utils.toWei('70', 'ether'), {
        value: web3.utils.toWei('10', 'ether'),
      });

      // account[1] 사용자가 5이더를 Exchange 컨트랙트에게 WEMEX 토큰 5개로 바꿔달라고 한다.
      await exchange.ethToERC20CSMMSwap({
        from: accounts[1],
  ...
  describe.skip('CSMM', async () => {
    it('가격은 일정해야한다.', async () => {
      ...
      let exchangeBalance = await exchange.getBalance();
      var balance = await token.balanceOf(exchange.address);
      let price = await exchange.getPrice(exchangeBalance, balance);
  ...
  describe.skip('CPMM', async () => {
    it('단순 슬리피지 계산', async () => {
      await token.approve(exchange.address, web3.utils.toWei('50', 'ether'));
      // 유동성 공급자가 40개의 토큰과 10개의 이더리움을 추가로 공급했다. (총 27개 이더 보유/115개 토큰 보유)
      await exchange.addLiquidity(web3.utils.toWei('40', 'ether'), {
        value: web3.utils.toWei('10', 'ether'),
      });
      ...
      let exchangeTokenBalance = await token.balanceOf(exchange.address);
      console.log(
        '거래소에는 몇개의 토큰? ->',
        web3.utils.toBN(exchangeTokenBalance).toString()
      );

      let temp = await exchange.getOuputAmount(
      ...
      let temp2 = await exchange.getOuputAmount(
  ...
  describe('CPMM', async () => {
    it('CPMM swap', async () => {
      await token.approve(exchange.address, toWei('4000', 'ether'));
      // 유동성 공급자가 40개의 토큰과 10개의 이더리움을 추가로 공급했다. (총 37개 이더 보유/155개 토큰 보유)
      await exchange.addLiquidity(toWei('40', 'ether'), {
      ...
      await exchange.ethToERC20CPMMSwap(toWei('0', 'ether'), {
        from: accounts[1],
        value: web3.utils.toWei('1', 'ether'),
      });
      console.log(accounts[1]);
      let temp = await token.balanceOf(accounts[1]);
      // console.log(toBN(temp).toString());
    });
  });
});
