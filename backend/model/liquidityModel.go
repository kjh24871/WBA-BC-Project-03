...
func (p *Model) AddLiquidity(_desiredAmountA int64, _desiredAmountB int64, pk string, token1Address string, token2Address string) *protocol.ApiResponse[any] {
	contracts, err := contracts.NewLiquidity(p.liquidityAddress, p.client)
	if err != nil {
		return protocol.Fail(err, 500)
	}
	tokenAAddress := common.HexToAddress(token1Address)
	tokenBAddress := common.HexToAddress(token2Address)
	...
	// TODO : Approve 2 토큰하기
	_, err = tokenAContracts.Approve(transactorOpts, p.liquidityAddress, inputAmountA)
	if err != nil {
		return protocol.Fail(err, 500)
	}
	_, err = tokenBContracts.Approve(transactorOpts, p.liquidityAddress, inputAmountB)
	if err != nil {
		return protocol.Fail(err, 500)
	}
...
func (p *Model) BalanceOf(strAddress string) *protocol.ApiResponse[any] {
